...
# input arguments
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:],"h:iq",["help","invid=","quarter="])
...
# extract metadata from CSV
    ...
    print '\n%4s %9s %7s %5s %2s %10s %10s %10s' %('#', 'KepID', 'InvID', 'KpMag', 'Md', 'Start', 'Stop', 'Release')
    kepid = []; invid = []; mode = []
    ra = []; dec = []; kepmag = []
    start = []; stop = []; release = []
    for line in lines:
        ...
        if (len(line[0]) > 0 and 
            ...
            if (JDstart > Qstart and JDstop < Qstop):
                ...
                print '%4d %9s %7s %5.2f %2s %10s %10s %10s' % (len(kepid), kepid[-1], invid[-1], kepmag[-1], mode[-1], start[-1][:10], stop[-1][:10], release[-1][:10])
    if (len(kepid) == 0):
